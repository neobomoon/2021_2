...
class Parser:
    ...
    def parse(self, con_code):
        ...
        if len(con_code) == 1 and self.isint(con_code[0]):
            return LFAEDS.Num( con_code[0] )

        # add
        elif con_code[0] == "+" and len( con_code ) == 3:
            return LFAEDS.Add( self.parse( con_code[1] ), self.parse( con_code[2] ) )

        # sub
        elif con_code[0] == "-" and len( con_code ) == 3:
            return LFAEDS.Sub( self.parse( con_code[1] ), self.parse( con_code[2] ) )

        # with
        elif con_code[0] == "with" and len( con_code ) == 3: # (fun p (parse b))
            id_val = self.split_con_code(con_code[1])
            return LFAEDS.App( LFAEDS.Fun( id_val[0], self.parse( con_code[2] ) ), self.parse( id_val[1] ) )

        #id (대소문자 영어만 가능)
        elif self.checkId( con_code[0] ) and len( con_code ) == 1 :
            return LFAEDS.Id( con_code[0] )

        # fun 선언
        elif con_code[0] == "fun" and len( con_code ) == 3:
            list_p = self.split_con_code(con_code[1])
            return LFAEDS.Fun( list_p[0], self.parse( con_code[2] ) )
        
        elif con_code[0] == "dsfun" and len( con_code ) == 3:
            list_p = self.split_con_code(con_code[1])
            return LFAEDS.Dsfun( list_p[0], self.parse( con_code[2] ) )
        
        #function call
        elif len( con_code ) == 2:
            return LFAEDS.App( self.parse( con_code[0] ), self.parse( con_code[1] ) )
